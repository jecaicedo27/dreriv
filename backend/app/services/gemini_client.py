"""
Gemini (Google) API client for AI-powered trading decisions
Gemini 2.5 Flash as alternative Layer 2 provider
"""
...
class GeminiTradingEngine:
    """
    Gemini client for trading decisions
    - Same interface as Groq/OpenAI/Claude
    - Uses Gemini 2.5 Flash for fast reasoning
    - JSON output via system instruction
    """
    ...
    def __init__(
                    ...
                    with open(env_path) as f:
                        for line in f:
                            if line.startswith('GEMINI_API_KEY='):
                                ...
                                logger.info(f"🔑 Gemini key loaded from .env file")
                                break
        ...
        logger.info(f"🔶 Gemini Trading Engine initialized - Model: {self.model_name}")
    ...
    async def get_decision(
        ...
        """
        Get AI trading decision - same interface as Groq/OpenAI/Claude
        """
        start = time.time()
        
        try:
            response = await asyncio.wait_for(
                self._call_gemini_api(system_prompt, market_context),
            ...
            logger.info(
                f"🔶 Gemini decision: {decision['decision']} "
            ...
            logger.warning(f"⏱️ Gemini timeout after {self.timeout}s")
    ...
    async def _call_gemini_api(self, system_prompt: str, user_message: str):
    ...
    def _validate_response(self, response: Dict[str, Any]) -> Dict[str, Any]:
        """Validate and sanitize - same logic as other providers"""
        errors = []
        warnings = []
        ...
        if errors:
            logger.warning(f"⚠️ Gemini response validation failed: {errors}")
...
def get_gemini_engine() -> GeminiTradingEngine:
    """Get or create Gemini engine singleton"""
    global gemini_engine
    if gemini_engine is None:
        gemini_engine = GeminiTradingEngine()
