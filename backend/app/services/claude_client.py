"""
Claude (Anthropic) API client for AI-powered trading decisions
Claude Sonnet 4 as alternative Layer 2 provider
"""
...
class ClaudeTradingEngine:
    """
    Claude client for trading decisions
    - Same interface as GroqTradingEngine / OpenAITradingEngine
    - Uses Claude Sonnet 4 for strong reasoning
    - JSON output via system prompt instruction
    """
    ...
    def __init__(
                    ...
                    with open(env_path) as f:
                        for line in f:
                            if line.startswith('CLAUDE_API_KEY='):
                                ...
                                logger.info(f"🔑 Claude key loaded from .env file")
                                break
        ...
        logger.info(f"🟣 Claude Trading Engine initialized - Model: {self.model}")
    ...
    async def get_decision(
        ...
        """
        Get AI trading decision - same interface as Groq/OpenAI
        """
        start = time.time()
        
        try:
            response = await asyncio.wait_for(
                self._call_claude_api(system_prompt, market_context),
            ...
            # Validate structure (same as Groq/OpenAI)
            ...
            logger.info(
                f"🟣 Claude decision: {decision['decision']} "
            ...
            logger.warning(f"⏱️ Claude timeout after {self.timeout}s")
    ...
    async def _call_claude_api(self, system_prompt: str, user_message: str):
    ...
    def _validate_response(self, response: Dict[str, Any]) -> Dict[str, Any]:
        """Validate and sanitize - same logic as Groq/OpenAI"""
        errors = []
        warnings = []
        ...
        if errors:
            logger.warning(f"⚠️ Claude response validation failed: {errors}")
...
def get_claude_engine() -> ClaudeTradingEngine:
    """Get or create Claude engine singleton"""
    global claude_engine
    if claude_engine is None:
        claude_engine = ClaudeTradingEngine()
