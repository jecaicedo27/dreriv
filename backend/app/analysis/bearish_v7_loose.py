"""
Bearish v7 Loose — Same as bearish_v6 but with 1.0% overextension threshold.

Clone of bearish_engine.py with ONLY ONE change:
  - Overextension gate: 0.5% → 1.0%

This allows the engine to enter PUT trades even when price has already
dropped further from EMA21, catching more of the trend at the cost of
potentially entering overextended moves.
"""
import pandas as pd
import numpy as np
...
class BearishLooseEngine(BaseAnalysisEngine):
    """
    Bearish v7 Loose: 1.0% overextension threshold (vs 0.5% in v6)
    """
    
    name = "bearish_v7"
    version = "7.1"
    description = "Bearish v7 Loose: 1.0% overextension threshold"
    ...
    # ============ THE ONLY DIFFERENCE ============
    OVEREXTENSION_THRESHOLD = 1.0  # was 0.5 in v6
    # =============================================
    ...
    def analyze(self, df: pd.DataFrame, symbol: str = "R_100", **kwargs) -> Dict[str, Any]:
        ...
        # ===== OVEREXTENSION CHECK (1.0% threshold) =====
